...
class OptionsGexResponse(BaseModel):
    ...
    whale_sweep_supported: Optional[bool] = None  # yfinance 沒有逐筆成交數據，固定為 False；見 yfinance_client.py 說明
    updated_at: Optional[str] = None
...
class OptionsState:
    ...
    def __init__(self) -> None:
        ...
        self.whale_sweep_supported: bool = False  # yfinance 沒有逐筆成交數據，固定為 False
        self.last_updated: Optional[str] = None
...
# 犧牲：yfinance 沒有逐筆成交數據，期權大單即時流（whale_sweep_feed）目前
# 恆為空，whale_sweep_supported 恆為 False，前端已有對應的「不支援」畫面。
...
def load_state_snapshot() -> None:
    ...
    try:
        ...
        for symbol, data in snapshot.get("options_states", {}).items():
            ...
            opt_state.whale_sweep_supported = data.get("whale_sweep_supported", False) or False
...
async def get_options_gex() -> OptionsGexListResponse:
    ...
    return OptionsGexListResponse(
        underlyings=underlyings, data_source_ok=data_source_ok,
        updated_at=datetime.now(timezone.utc).isoformat(),
    )
...
async def get_options_whale_sweep() -> WhaleSweepResponse:
    """期權大單即時流（實驗性；目前資料源 yfinance 沒有逐筆成交數據，恆回傳空清單，見 yfinance_client.py 說明）。"""
    async with state.lock:
