...
import gex_engine
from moomoo_client import MoomooOptionsClient, WhaleSweepListener
...
# --- 📊 期權分析（Options Analytics，獨立模塊：Moomoo/Futu OpenD 期權鏈 ->
# Black-Scholes GEX 計算引擎 -> Gamma 擠壓臨界點）---
# ⚠️ 這個模塊需要真實的 Moomoo/Futu 證券帳戶 + 自行啟動的 OpenD 閘道程式，
# 不是單純設一組 API Key 就能用（見 moomoo_client.py 開頭說明）。預設關閉，
# 避免還沒設好 OpenD 就啟動背景迴圈狂連線失敗、洗爆 Telegram 失敗告警。
# 設定 OPTIONS_ANALYTICS_ENABLED=true 才會真的啟動這個背景迴圈。
OPTIONS_ANALYTICS_ENABLED = os.environ.get("OPTIONS_ANALYTICS_ENABLED", "").strip().lower() == "true"
MOOMOO_OPEND_HOST = os.environ.get("MOOMOO_OPEND_HOST", "127.0.0.1").strip()
MOOMOO_OPEND_PORT = int(os.environ.get("MOOMOO_OPEND_PORT", "11111"))

# 強制首批監控標的（Futu/Moomoo 美股代碼格式為 "US.<TICKER>"）
...
class OptionsGexResponse(BaseModel):
    ...
    whale_sweep_supported: Optional[bool] = None  # None=尚未嘗試訂閱過；見 moomoo_client.py 說明
    updated_at: Optional[str] = None
...
class OptionsState:
    ...
    def __init__(self) -> None:
        ...
        self.whale_sweep_supported: Optional[bool] = None  # None=尚未嘗試訂閱過
        self.last_updated: Optional[str] = None
...
class AppState:
    def __init__(self) -> None:
        ...
        self.whale_sweep_feed: Deque[dict] = deque(maxlen=OPTIONS_WHALE_SWEEP_FEED_MAX_LEN)
        self.last_options_scan_at: float = 0.0  # time.monotonic() 時間戳
        self.moomoo_connected: bool = False      # 給監控/前端顯示「Moomoo資料源目前是否連線中」
...
def _parse_whale_sweep_event(code: str, price: float, volume: float) -> Optional[dict]:
    """
    從期權合約代碼（如 US.NVDA260116C150000）解析出到期日/Call-Put/履約價。
    這是 Futu 生態圈常見的代碼格式慣例，非官方文件明列規範，第一次收到真實
    tick 後應核對一次；解析不出來就回傳 None，呼叫端會直接跳過該筆事件，
    不影響其他事件或讓迴圈掛掉。
    """
    match = _OPTION_CODE_PATTERN.match(code)
    if not match:
        return None
    date_str, cp, strike_raw = match.groups()
    try:
        expiry_date = datetime.strptime(date_str, "%y%m%d").date()
    except ValueError:
        return None

    symbol = next((name for name, us_code in OPTIONS_UNDERLYINGS.items() if code.startswith(us_code + "")), None)
    if symbol is None:
        return None
    ...
    return {
        ...
        "premium_usd": price * volume * 100,  # 每口合約對應100股
    }


async def _drain_whale_sweep_events(whale_listener: WhaleSweepListener) -> None:
    """把 WhaleSweepListener 累積的 tick 事件轉成大單流紀錄，只留達門檻的成交。"""
    events = whale_listener.drain()
    if not events:
        return
    ...
    for df in events:
        for _, row in df.iterrows():
            try:
                ...
                parsed = _parse_whale_sweep_event(str(row.get("code", "")), price, volume)
                if parsed is None or parsed["premium_usd"] < OPTIONS_WHALE_SWEEP_MIN_PREMIUM_USD:
                    continue

                direction = str(row.get("ticker_direction", "")).upper()
                if direction not in ("BUY", "SELL"):
                    continue  # 方向不明的成交，寧可跳過不顯示，不猜測方向誤導使用者
                ...
                logger.warning("解析期權大單 tick 失敗：%s", exc)
...
async def scan_options_analytics(client: MoomooOptionsClient, whale_listener: WhaleSweepListener) -> None:
    """
    對每個 OPTIONS_UNDERLYINGS 標的：拉最近到期日的完整期權鏈 + 現貨價，交給
    gex_engine 算 Net GEX 剖面跟 Gamma 擠壓臨界點。第一次拿到期權代碼清單後，
    順便嘗試訂閱大單 TICKER（只嘗試一次，失敗就設 supported=False 不再重試，
    避免每輪重複打一個已知不支援的訂閱）。
    """
    now_iso = datetime.now(timezone.utc).isoformat()
    all_option_codes: List[str] = []

    for display_name, us_code in OPTIONS_UNDERLYINGS.items():
        try:
            spot = await client.get_spot_price(us_code)
            expiry = await client.get_nearest_expiry(us_code)
            if spot is None or expiry is None:
                continue

            legs_raw = await client.get_option_chain_legs(us_code, expiry)
            if not legs_raw:
                continue

            all_option_codes.extend(
                code for leg in legs_raw for code in (leg.call_code, leg.put_code) if code
            )
    ...
    if whale_listener.supported is None and all_option_codes:
        await whale_listener.try_subscribe(all_option_codes)
        async with state.lock:
            for opt_state in state.options_states.values():
                opt_state.whale_sweep_supported = whale_listener.supported

    await _drain_whale_sweep_events(whale_listener)
...
async def options_analytics_loop() -> None:
    """
    背景永久迴圈：只在 OPTIONS_ANALYTICS_ENABLED=true 時真正運作（預設關閉，
    見常數定義處說明）。美股盤中每 OPTIONS_SCAN_INTERVAL_SECONDS 秒重算一次
    GEX剖面；非盤中降到 OPTIONS_CLOSED_POLL_SECONDS 秒檢查一次「開盤了沒」。
    OpenD 連線斷掉時會整個重新 connect，不會卡死其他背景迴圈。
    """
    if not OPTIONS_ANALYTICS_ENABLED:
        logger.info("OPTIONS_ANALYTICS_ENABLED 未開啟，期權分析背景迴圈不啟動")
        return

    client = MoomooOptionsClient(host=MOOMOO_OPEND_HOST, port=MOOMOO_OPEND_PORT)
    whale_listener = WhaleSweepListener(client)
    tz = ZoneInfo(US_MARKET_TZ)
    ...
    while True:
        ...
        if not _is_us_market_active(now_et):
            async with state.lock:
                state.moomoo_connected = client.is_connected
        ...
        try:
            if not client.is_connected:
                ...
                async with state.lock:
                    state.moomoo_connected = connected
                if not connected:
                    raise RuntimeError("無法連線 Moomoo OpenD")

            await scan_options_analytics(client, whale_listener)
            async with state.lock:
                state.moomoo_connected = True
            failure_notification = _record_loop_outcome("期權分析迴圈", success=True)
        except Exception as exc:  # noqa: BLE001 - 背景迴圈需持續存活，統一捕捉並記錄錯誤
            logger.error("期權分析背景迴圈發生錯誤：%s", exc)
            await client.close()  # 連線可能已經壞掉，下一輪重新建立，不要卡在一個壞掉的連線上
            async with state.lock:
        ...
        snapshot = {
            ...
            "options_states": {
                symbol: {
                    ...
                    "whale_sweep_supported": o.whale_sweep_supported,
                }
...
def load_state_snapshot() -> None:
    ...
    try:
        ...
        for symbol, data in snapshot.get("options_states", {}).items():
            ...
            opt_state.whale_sweep_supported = data.get("whale_sweep_supported")
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        options_task.cancel()
        for task in (monitor_task, us_stock_task, news_agent_task, squeeze_mode_task, options_task):
            try:
                await task
...
async def get_options_gex() -> OptionsGexListResponse:
    """
    📊 期權分析（獨立、實驗性模塊，需要 Moomoo/Futu OpenD 連線）：所有強制
    監控標的的 Net GEX 剖面 + Gamma 擠壓臨界點。has_data=False 代表還沒連上
    OpenD 或還沒拉到資料，不是「這檔沒有擠壓」。
    """
    async with state.lock:
        underlyings = []
        ...
        moomoo_connected = state.moomoo_connected
    return OptionsGexListResponse(
        underlyings=underlyings, moomoo_connected=moomoo_connected,
        updated_at=datetime.now(timezone.utc).isoformat(),
    )
