...
async def get_candles(symbol: str, limit: int = 60) -> CandlesListResponse:
    """
    回傳指定標的最近已收盤的真實K線，供前端畫真實蠟燭圖用（取代原本 demo 用的
    假隨機漫步資料）。跟主流幣策略共用同一支 fetch_ohlcv_for_symbol，資料來源
    一致；不限定只能查 MAJOR_SYMBOLS，市場掃描/迷因幣的標的也能查。
    """
    ...
    try:
        df = await fetch_ohlcv_for_symbol(exchange_pool_ref, symbol, limit=limit)
    except Exception as exc:  # noqa: BLE001
