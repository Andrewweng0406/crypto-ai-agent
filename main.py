...
# --- 🎯 RSI(2) 均值回歸實盤監控（獨立模塊，見 8.7 節完整定義；策略邏輯/回測
# 引擎在網頁端回測沙盒那個區塊，這裡只放實盤監控迴圈需要的常數）---
RSI2_SCAN_INTERVAL_SECONDS = 60   # 每分鐘重抓一次日線（yfinance盤中的「今天」那根K棒是即時跳動的，藉此達到即時監控效果，不需要另外接報價流）
RSI2_HISTORY_MAX_LEN = 30
...
# --- 🏦 美股多機構研究彙總（獨立模塊，本機 moomoo_research_sync_local.py 主動回傳）---
# 跟 whale sweep/清算牆共用同一組 WHALE_SWEEP_API_KEY。資料源是 moomoo/futu-api
# 的 get_research_analyst_consensus / get_research_rating_summary /
# get_research_morningstar_report 三支API（2026-07-17驗證：這三支對美股回傳的是
# 真實機構評級與晨星研報內容，不是憑空生成，見 moomoo_client.py 說明），
# 涵蓋標的為 state.us_stock_watchlist，不另外維護一份清單。
# 評級數值定義來自 futu SDK 的 Qot_Common.ResearchRatingType（美股只會出現
# 1/3/4，2和5是非美市場才有的中間值，保留在對照表裡以防未來擴充）：
RESEARCH_RATING_LABELS: Dict[int, str] = {1: "賣出", 2: "表現不佳", 3: "持有", 4: "買入", 5: "強力買入"}
...
class InstitutionRatingItem(BaseModel):
    ...
    rating: Optional[int] = None  # 見 RESEARCH_RATING_LABELS 對照表
    target_price: Optional[float] = None
    recommendation_date_str: Optional[str] = None
    rating_url: Optional[str] = None  # 該筆評級的原始新聞來源連結（TipRanks）


class ResearchIngestRequest(BaseModel):
    """本機 moomoo_research_sync_local.py 主動 POST 回傳雲端的機構研究彙總快照。"""

    symbol: str
    consensus_high: Optional[float] = None
    consensus_average: Optional[float] = None
    consensus_low: Optional[float] = None
    consensus_total: Optional[int] = None
    consensus_rating: Optional[int] = None  # 見 RESEARCH_RATING_LABELS 對照表
    consensus_buy_pct: Optional[float] = None
    consensus_hold_pct: Optional[float] = None
    consensus_sell_pct: Optional[float] = None
    institution_ratings: List[InstitutionRatingItem] = []
    morningstar_star_rating: Optional[int] = None
    morningstar_fair_value: Optional[float] = None
    morningstar_fair_value_context: Optional[str] = None
    morningstar_moat_label: Optional[str] = None
    morningstar_moat_context: Optional[str] = None
    morningstar_uncertainty_label: Optional[str] = None
    morningstar_financial_health_label: Optional[str] = None
...
class ResearchBundleData(BaseModel):
    symbol: str
    has_data: bool
    consensus_high: Optional[float] = None
    consensus_average: Optional[float] = None
...
class ResearchBundlesResponse(BaseModel):
    underlyings: List[ResearchBundleData]
    updated_at: Optional[str] = None
...
class RSI2StockState:
    """
    單一標的的 RSI(2) 均值回歸實盤監控狀態（獨立狀態，跟其他所有模塊完全分開）。
    sma200/sma5/rsi2 在盤中是用「今天還在跳動的即時價格」當今天K棒的收盤估算值算出
    來的（rsi2_is_confirmed=False），只有收盤後才是真正確認值——這是刻意的：使用者
    要求要能在盤中持續看到現價跟技術線的關係，但真正的進場判斷永遠只認前一天已經
    確認收盤的訊號，不會被盤中還在跳動的估算值誤導進出場。
    """

    def __init__(self) -> None:
        self.current_price: Optional[float] = None
        self.day_change_pct: Optional[float] = None
        self.sma200: Optional[float] = None
        self.sma5: Optional[float] = None
...
class AppState:
    def __init__(self) -> None:
        ...
        # 🏦 美股多機構研究彙總（獨立狀態，本機 moomoo_research_sync_local.py 主動
        # 回傳；跟清算牆同樣的「整份覆蓋、不逐筆累加」模式，因為本機端已彙總好）
        self.research_bundles: Dict[str, dict] = {}
        ...
        # 🎯 RSI(2) 均值回歸實盤監控（獨立狀態，跟其他所有模塊完全分開；只涵蓋
    ...
    def get_rsi2_state(self, symbol: str) -> RSI2StockState:
        if symbol not in self.rsi2_states:
            self.rsi2_states[symbol] = RSI2StockState()
        return self.rsi2_states[symbol]
...
# ---------------------------------------------------------------------------
# 6.95 🎯 RSI(2) 均值回歸實盤監控（獨立模塊，2026-07-11 從回測沙盒驗證過的
# stock_rsi2_meanrev 策略移植成正式實盤監控——只監控、記錄訊號跟通知，不是自動
# 下單。策略/模擬引擎（_backtest_signals_rsi2_mean_reversion 等）定義在下面
# 8.5 網頁端回測沙盒那個區塊，這裡直接呼叫共用，不重複實作一份。
# ---------------------------------------------------------------------------


async def scan_rsi2_stock(display_name: str) -> None:
    """
    單一標的的RSI(2)均值回歸監控：每輪都重抓一次日線（yfinance盤中「今天」那根
    K棒本身就是即時跳動的，藉此不必另外接報價流就能做到盤中即時監控）。今天這根
    K棒算出來的sma200/sma5/rsi2是「即時估算值」（rsi2_is_confirmed=False，收盤前
    都可能隨現價變動），但進場判斷永遠只看昨天已經確認收盤的訊號
    （yesterday["entry_signal"]），不會被盤中還在跳動的估算值誤導進出場——完全
    比照策略本身「訊號收盤確認、隔天開盤進場」的設計。
    """
    try:
        raw = await asyncio.to_thread(_backtest_fetch_yf_daily, display_name, STOCK_MEANREV_FETCH_YEARS)
    except Exception as exc:  # noqa: BLE001
        logger.warning("RSI2均值回歸抓取 %s 日線失敗：%s", display_name, exc)
        return
    if len(raw) < 210:
        logger.warning("RSI2均值回歸 %s 日線資料不足（%d根），跳過本輪", display_name, len(raw))
        return

    signals = _backtest_signals_rsi2_mean_reversion(raw)
    today = signals.iloc[-1]
    yesterday = signals.iloc[-2]
    prev_close = float(signals.iloc[-2]["close"])
    current_price = float(today["close"])
    now_iso = datetime.now(timezone.utc).isoformat()

    settlement_notification: Optional[str] = None
    entry_notification: Optional[str] = None

    async with state.lock:
        st = state.get_rsi2_state(display_name)
        st.current_price = current_price
        st.day_change_pct = (current_price - prev_close) / prev_close * 100 if prev_close else None
        st.sma200 = float(today["sma200"]) if pd.notna(today["sma200"]) else None
        st.sma5 = float(today["sma5"]) if pd.notna(today["sma5"]) else None
        ...
        if st.open_signal is not None:
            ...
            if hit_sl or hit_tp:
                ...
                pnl_pct = (exit_price - side["entry_price"]) / side["entry_price"] * 100
                result = "WIN" if pnl_pct > 0 else "LOSS"
                closed_record = {
                    "symbol": display_name,
                    "display_name": display_name,
                    "side": "Long",
                    "entry_price": side["entry_price"],
                    "exit_price": exit_price,
                    "stop_loss": side["stop_loss"],
                    "result": result,
                    "pnl_pct": pnl_pct,
                    "opened_at": side["opened_at"],
                    "closed_at": now_iso,
                ...
                settlement_notification = (
                    f"{emoji} <b>RSI(2)均值回歸 結算</b>\n{display_name} Long\n"
                    f"結果：{result}（{'停損' if hit_sl else '止盈(收復SMA5)'}）\n"
        ...
        if st.open_signal is None and bool(yesterday["entry_signal"]) and pd.notna(today["open"]) and already_traded_today:
            ...
            if entry_price > stop_loss:  # 理論上恆成立（策略本身的止損設計），防呆用
                ...
                entry_notification = (
                    ...
                    f"進場：{entry_price:.2f}（開盤價）\n停損：{stop_loss:.2f}\n"
                    f"止盈：收盤收復SMA5（動態，目前約 {sma5_hint}）"
                )

        st.last_updated = now_iso

    if settlement_notification:
        await send_telegram_message(settlement_notification)
        await push_assistant_broadcast(settlement_notification.split("\n")[1], display_name, "rsi2_meanrev")
    if entry_notification:
        await send_telegram_message(entry_notification)
        await push_assistant_broadcast(f"🎯 {display_name} RSI(2)均值回歸觸發進場訊號", display_name, "rsi2_meanrev")


async def rsi2_meanrev_loop() -> None:
    """
    RSI(2)均值回歸背景迴圈：只在美股交易時段運作（盤前/盤後/假日整個睡眠，不打
    任何API）。不像美股ORB是當沖策略、收盤要強制平倉——這是swing策略，平均持倉
    1-3天，部位可以跨日繼續持有，收盤時什麼都不用做。
    """
    tz = ZoneInfo(US_MARKET_TZ)
    ...
    while True:
        ...
        await asyncio.sleep(RSI2_SCAN_INTERVAL_SECONDS)
...
def load_state_snapshot() -> None:
    ...
    try:
        ...
        state.research_bundles.update(snapshot.get("research_bundles", {}))
...
STOCK_MEANREV_FETCH_YEARS = 7           # 本機驗證抓的是2020-01-01至今（約6.5年），這裡抓7年留緩衝，
                                         # 避免yf.Ticker().history(period=...)相對窗口比驗證用的固定
                                         # 起始日短一點，篩掉本該存在的早期交易樣本


class BacktestRequest(BaseModel):
    symbol: str
    strategy_name: Literal[
        "crypto_donchian_4h", "meme_volume_spike", "us_stock_orb", "supertrend_btc_long", "stock_rsi2_meanrev"
    ]
    days_range: int = 30
...
def _backtest_fetch_yf_daily(ticker: str, years: int) -> pd.DataFrame:
    """
    美股日線K線，不像15分鐘線有60天硬上限——yfinance日線資料可以往回抓好幾年，
    是「高勝率策略」（RSI2均值回歸等，見STOCK_HIGH_WINRATE_CAVEAT）需要長歷史
    暖機SMA200+累積足夠交易樣本的必要條件。auto_adjust=True取股息/分割調整後
    收盤價，是回測的標準做法（不調整的話除息日會被誤判成一次假跳空）。
    """
    raw = yf.Ticker(ticker).history(period=f"{years}y", interval="1d", auto_adjust=True)
    if raw.empty:
        return pd.DataFrame(columns=["timestamp", "open", "high", "low", "close", "volume"])
    return pd.DataFrame({
        "timestamp": (raw.index.view("int64") // 10**6),
...
def _backtest_signals_donchian_4h(df: pd.DataFrame) -> pd.DataFrame:
    """
    2026-07-14修復：原本這裡是「1H唐奇安20+EMA20/60」，但正式上線的 deep_scan_symbol／
    detect_new_signal 用的其實是「4H唐奇安20+SMA(MA_FAST_PERIOD)/SMA(MA_SLOW_PERIOD)」
    ——名字很像，但K線週期跟趨勢濾網週期/類型都不一樣，等於這個回測沙盒從來沒有真正
    驗證過實盤在跑的策略。這裡改成跟 add_indicators()／detect_new_signal() 逐項對齊
    （唐奇安週期、成交量均值窗口、成交量倍數、ATR公式全部沿用同一組常數/同一個
    _compute_atr_series，趨勢濾網改成SMA），呼叫端也要把抓取的K線timeframe從
    "1h" 改成 "4h"，不然這裡的訊號時間軸會跟資料對不上。
    """
    df = df.copy()
    df["atr"] = _compute_atr_series(df)
    df = _backtest_add_trend_variant(df, fast=MA_FAST_PERIOD, slow=MA_SLOW_PERIOD, use_ema=False)
    ...
    volume_confirmed = df["volume"] > avg_volume * VOLUME_MULT
...
# ---------------------------------------------------------------------------
# 📈 高勝率股票策略：RSI(2) 均值回歸（Larry Connors經典）
# ---------------------------------------------------------------------------
# 來源：2026-07-11 在本機標的資料夾對10檔標的(BTC/ETH/SOL+7檔美股大型科技)跑近
# 6年日線歷史資料的批次測試，NVDA/GOOGL/MSFT/META/AAPL 五檔都做到65%+勝率，
...
def _compute_rsi_series(close: pd.Series, length: int) -> pd.Series:
    """手刻RSI（Wilder's RMA平滑），2026-07-11已用NVDA近6年日線資料逐點比對過
    pandas_ta.rsi()輸出：1637筆資料完全零誤差。"""
    delta = close.diff(1)
...
def _backtest_signals_rsi2_mean_reversion(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["sma200"] = df["close"].rolling(200).mean()
    df["sma5"] = df["close"].rolling(5).mean()
    df["rsi2"] = _compute_rsi_series(df["close"], 2)
    df["entry_signal"] = ((df["close"] > df["sma200"]) & (df["rsi2"] < 10)).fillna(False)
    df["sl_price"] = df["close"] * (1 - STOCK_MEANREV_SL_PCT)
    return df


def _stock_meanrev_leverage(stop_loss_pct: float) -> float:
    """
    故意不設下限1.0——固定止損距離通常換算出來就是<1倍的部位大小（用比較保守的
    資金比例進場），這是這套策略「低回撤」特性的來源，不該被人為拉高到1倍以上。
    """
    if stop_loss_pct <= 0:
        return 0.0
    return min(STOCK_MEANREV_MAX_LEVERAGE, STOCK_MEANREV_RISK_PCT / stop_loss_pct)


def _backtest_simulate_indicator_cross(df: pd.DataFrame, tp_col: str, start_idx: int) -> List[dict]:
    """
    止盈是「收盤價穿越某個指標」而不是固定價位（例如穿越SMA5），跟 _backtest_simulate_htf
    的固定TP/SL、_backtest_simulate_supertrend的追蹤停損都不同，是第三種出場機制——
    完全比照 long_only_engine.py 的 LongOnlyBacktester 邏輯：止損是訊號根算好的固定
    價位，止盈是逐根用當根指標值動態判斷，兩者同根都觸發時止損優先（保守判定）。
    """
    trades: List[dict] = []
    open_position: Optional[dict] = None
    ...
    for i in range(start_idx, len(df)):
        ...
        if open_position is not None:
            ...
            tp_val = candle[tp_col]
            hit_tp = pd.notna(tp_val) and candle["close"] > tp_val

            if hit_sl or hit_tp:
                exit_price = sl_price if hit_sl else float(candle["close"])
                raw_pnl_pct = (exit_price - open_position["entry_price"]) / open_position["entry_price"] * 100
                pnl_pct = raw_pnl_pct * open_position["leverage"]
                fee_cost_pct = 2 * BACKTEST_CRYPTO_FEE_PCT_PER_SIDE * open_position["leverage"]
                pnl_pct_after_fee = pnl_pct - fee_cost_pct
        ...
        if i > 0 and bool(df.iloc[i - 1]["entry_signal"]):
            entry_price = float(candle["open"])
            sl_price = float(df.iloc[i - 1]["sl_price"])
            ...
            if stop_loss_pct > MAX_SANE_STOP_LOSS_PCT:
                continue
            leverage = _stock_meanrev_leverage(stop_loss_pct)
            open_position = {
...
async def get_smart_money(symbol: str = MAJOR_SYMBOLS[0]) -> SmartMoneyResponse:
    """
    回傳指定標的目前的合約聰明錢數據快照與偏見判斷。這份數據（資金費率/OI變化/
    大戶多空比）只有 refresh_major_smart_money() 會算，該函式只跑 MAJOR_SYMBOLS
    （BTC/ETH/SOL）——迷因幣（WIF/DOGE等）不在範圍內，不是「還沒抓到」，是這個
    標的本來就不會有這份數據，兩種情況要回不同的錯誤訊息，不能都說「稍後再試」
    誤導使用者一直重試一個永遠不會成功的請求（2026-07-11實測發現：DOGE查這個
    端點會拿到跟「暫時故障」語意一樣的503，但其實是永久性的範圍限制）。
    """
    if symbol not in MAJOR_SYMBOLS:
        raise HTTPException(
            status_code=404,
            detail=f"{symbol} 不在聰明錢數據追蹤範圍內（目前只追蹤主流幣 {', '.join(MAJOR_SYMBOLS)}），"
                   f"不是暫時性故障，重試不會有結果。",
        )
...
@app.post("/api/ingest/research", response_model=ResearchIngestResponse)
async def ingest_research(
    item: ResearchIngestRequest, x_api_key: Optional[str] = Header(None, alias="X-API-Key")
) -> ResearchIngestResponse:
    """
    本機 moomoo_research_sync_local.py 主動回傳雲端的機構研究彙總快照接收端點。
    跟whale sweep/清算牆共用同一組 WHALE_SWEEP_API_KEY 驗證。研究資料本身更新
    頻率低（機構評級/晨星報告通常一天內頂多變動幾次），直接整份覆蓋掉該標的
    的舊快照，不需要像大單流那樣逐筆累加。
    """
    if WHALE_SWEEP_API_KEY is None or x_api_key != WHALE_SWEEP_API_KEY:
        raise HTTPException(status_code=401, detail="無效或缺少 X-API-Key")
    ...
    symbol = item.symbol.strip().upper()
    payload = item.model_dump(exclude={"symbol"})
...
@app.get("/api/us/research-bundles", response_model=ResearchBundlesResponse)
async def get_research_bundles() -> ResearchBundlesResponse:
    """
    🏦 美股多機構研究彙總（獨立、實驗性模塊）：資料源自使用者本機執行的
    moomoo_research_sync_local.py（透過 moomoo OpenD 的
    get_research_analyst_consensus / get_research_rating_summary /
    get_research_morningstar_report 三支API），涵蓋標的為美股ORB關注清單跟
    期權分析關注清單的聯集（沿用 _us_stock_news_tickers() 既有的這組聯集邏輯，
    兩邊清單的key本來就是真實美股代號，跟BingX代幣化商品符號分開）。
    has_data=False代表本機還沒回傳過這個標的的快照，不是「這個標的沒有機構覆蓋」。
    """
    async with state.lock:
        symbols = sorted(_us_stock_news_tickers())
        underlyings = []
        for symbol in symbols:
            data = state.research_bundles.get(symbol)
            if data is None:
                underlyings.append(ResearchBundleData(symbol=symbol, has_data=False))
                continue
            underlyings.append(ResearchBundleData(
                symbol=symbol,
                has_data=True,
                consensus_rating_label=RESEARCH_RATING_LABELS.get(data.get("consensus_rating")),
                **data,
            ))
    return ResearchBundlesResponse(underlyings=underlyings, updated_at=datetime.now(timezone.utc).isoformat())
...
class RSI2StockResponse(BaseModel):
    symbol: str
    display_name: str
    current_price: Optional[float] = None
    day_change_pct: Optional[float] = None
    ...
    rsi2_is_confirmed: bool = False
    status: Literal["NO_SIGNAL", "OPEN"] = "NO_SIGNAL"
    entry_price: Optional[float] = None
    stop_loss: Optional[float] = None
    opened_at: Optional[str] = None
    updated_at: Optional[str] = None
...
class RSI2ListResponse(BaseModel):
    market_session: Literal["OPEN", "CLOSED"]
    stocks: List[RSI2StockResponse]
    caveat: str
    updated_at: str


class RSI2HistoryItem(BaseModel):
    symbol: str
    display_name: str
    side: str
    entry_price: float
...
class RSI2HistoryStats(BaseModel):
    total_trades: int
    wins: int
    losses: int
    win_rate_pct: float
...
class RSI2HistoryResponse(BaseModel):
    trades: List[RSI2HistoryItem]
    stats: RSI2HistoryStats


class RSI2ChartPoint(BaseModel):
    timestamp: int
    open: float
    high: float
    low: float
...
class RSI2ChartResponse(BaseModel):
    symbol: str
    points: List[RSI2ChartPoint]


def _to_rsi2_response(display_name: str) -> RSI2StockResponse:
    """讀取單一標的目前狀態並組成 RSI2StockResponse，呼叫端須持有 state.lock。"""
    st = state.rsi2_states.get(display_name)
    if st is None:
        return RSI2StockResponse(symbol=display_name, display_name=display_name)

    signal = st.open_signal
    return RSI2StockResponse(
        symbol=display_name,
        display_name=display_name,
        current_price=st.current_price,
        day_change_pct=st.day_change_pct,
        sma200=st.sma200,
        sma5=st.sma5,
        rsi2=st.rsi2,
        rsi2_is_confirmed=st.rsi2_is_confirmed,
        status="OPEN" if signal else "NO_SIGNAL",
        entry_price=signal["entry_price"] if signal else None,
        stop_loss=signal["stop_loss"] if signal else None,
        opened_at=signal["opened_at"] if signal else None,
        updated_at=st.last_updated,
    )


@app.get("/api/rsi2-meanrev", response_model=RSI2ListResponse)
async def get_rsi2_meanrev() -> RSI2ListResponse:
    """🎯 RSI(2)均值回歸實盤監控（獨立模塊）：固定回傳 STOCK_MEANREV_SYMBOLS 全部標的的狀態。"""
    now_et = datetime.now(ZoneInfo(US_MARKET_TZ))
    async with state.lock:
        stocks = [_to_rsi2_response(name) for name in STOCK_MEANREV_SYMBOLS]

    return RSI2ListResponse(
        market_session="OPEN" if _is_us_market_active(now_et) else "CLOSED",
        stocks=stocks,
...
@app.get("/api/rsi2-meanrev/history", response_model=RSI2HistoryResponse)
async def get_rsi2_meanrev_history() -> RSI2HistoryResponse:
    ...
    return RSI2HistoryResponse(
        trades=trades,
        stats=RSI2HistoryStats(total_trades=total, wins=wins, losses=losses, win_rate_pct=round(win_rate, 2)),
    )


@app.get("/api/rsi2-meanrev/chart", response_model=RSI2ChartResponse)
async def get_rsi2_meanrev_chart(symbol: str = "NVDA") -> RSI2ChartResponse:
    """
    給前端技術面圖表用：近120個交易日的OHLC + SMA200/SMA5/RSI2序列——不是給
    使用者自己選任意標的，只開放STOCK_MEANREV_SYMBOLS這5檔已驗證過的。
    """
    symbol_raw = symbol.strip().upper()
    if symbol_raw not in STOCK_MEANREV_SYMBOLS:
        raise HTTPException(status_code=400, detail=f"只提供 {', '.join(STOCK_MEANREV_SYMBOLS)} 的圖表資料")

    try:
        raw = await asyncio.to_thread(_backtest_fetch_yf_daily, symbol_raw, STOCK_MEANREV_FETCH_YEARS)
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(status_code=502, detail=f"抓取 {symbol_raw} 日線歷史失敗：{exc}")
    if raw.empty:
    ...
    points = [
        RSI2ChartPoint(
            ...
            sma5=float(row["sma5"]) if pd.notna(row["sma5"]) else None,
            rsi2=float(row["rsi2"]) if pd.notna(row["rsi2"]) else None,
        )
        for _, row in recent.iterrows()
    ]
    return RSI2ChartResponse(symbol=symbol_raw, points=points)
...
async def run_backtest(payload: BacktestRequest, request: Request) -> BacktestResponse:
    ...
    # stock_rsi2_meanrev 需要好幾年日線資料才有足夠SMA200暖機+交易樣本，用自己的上限，
    # 其餘策略沿用原本180天上限（那是給小時線/15分鐘線設計的，日線策略用同一個上限
    # 會直接砍掉這個策略需要的大部分歷史）。
    max_days_range = (
        STOCK_MEANREV_MAX_DAYS_RANGE if payload.strategy_name == "stock_rsi2_meanrev" else BACKTEST_MAX_DAYS_RANGE
    )
    days_range = max(1, min(payload.days_range, max_days_range))
    symbol_raw = payload.symbol.strip().upper()

    if payload.strategy_name in ("crypto_donchian_4h", "meme_volume_spike"):
        ccxt_symbol = symbol_raw if "/" in symbol_raw else f"{symbol_raw}/USDT:USDT"
        is_donchian = payload.strategy_name == "crypto_donchian_4h"
        fetch_timeframe = "4h" if is_donchian else "1h"
        # 暖機緩衝：均線/ATR/RVOL都需要額外歷史資料才能算出第一筆有效訊號，沒有這個
        # 緩衝，回測窗口一開始的訊號會因為指標還是NaN而被跳過。crypto_donchian_4h
        # 2026-07-14對齊實盤後改用SMA(MA_SLOW_PERIOD=200)，在4H線上需要200*4=800小時
        # （約33天）才能暖機完成，原本給EMA(60)設計的+10天緩衝完全不夠，會讓回測窗口
        # 前段的訊號被靜悄悄跳過——這正是今晚pagination bug/MA200暖機bug同一類問題，
        # 這裡直接把修法內建進來，meme_volume_spike沒有長週期均線，維持原本的緩衝。
        fetch_days = days_range + (40 if is_donchian else 10)
        try:
            raw = await _backtest_fetch_crypto_history(ccxt_symbol, fetch_days, fetch_timeframe)
        ...
        if is_donchian:
            signals = _backtest_signals_donchian_4h(raw)
    ...
    if payload.strategy_name == "stock_rsi2_meanrev":
        if symbol_raw not in STOCK_MEANREV_SYMBOLS:
            raise HTTPException(
                status_code=400,
                detail=f"RSI(2)均值回歸目前只驗證過 {', '.join(STOCK_MEANREV_SYMBOLS)}，"
                       f"其他標的的結果沒有任何驗證基礎，不提供。",
            )
        try:
            raw = await asyncio.to_thread(_backtest_fetch_yf_daily, symbol_raw, STOCK_MEANREV_FETCH_YEARS)
        ...
        return BacktestResponse(
            ...
            sample_sufficient=summary["total_trades"] >= BACKTEST_MIN_TRADES_FOR_CONFIDENCE,
            days_range_requested=payload.days_range, days_range_used=days_range,
            data_source="yfinance (日線)", strategy_caveat=STOCK_MEANREV_CAVEAT, **summary,
        )
...
# --- 股票版 Walk-Forward 穩健性驗證（RSI2均值回歸專用）---
# 跟上面 supertrend 的滾動式訓練/測試不同：RSI2策略本身沒有可調參數要優化，
# 這裡驗證的是「同一套固定邏輯」在不同時間切片上是否普遍成立，不是「換時間窗
# 找新最佳參數」。完全比照桌面 walk_forward_batch.py 的方法論：整段資料依列數
# 切成前後兩半+四等分季度，全部用同一組固定策略邏輯跑，不重新搜參數。

class StockWalkForwardFold(BaseModel):
    fold: str
    start: str
    end: str
    total_trades: int
...
class StockWalkForwardResponse(BaseModel):
    strategy_name: str = "stock_rsi2_meanrev"
    symbol: str
    folds: List[StockWalkForwardFold]
    all_quarters_pass_50pct: bool
    min_quarter_win_rate: float
    caveat: str


def _stock_meanrev_run_fold(df: pd.DataFrame, label: str) -> StockWalkForwardFold:
    trades = _backtest_simulate_indicator_cross(df.reset_index(drop=True), "sma5", 0)
    summary = _backtest_summarize(trades)
    return StockWalkForwardFold(
        fold=label,
        start=str(pd.to_datetime(df["timestamp"].iloc[0], unit="ms").date()),
        end=str(pd.to_datetime(df["timestamp"].iloc[-1], unit="ms").date()),
        total_trades=summary["total_trades"],
        win_rate=summary["win_rate"],
        total_return_pct=round(summary["equity_curve"][-1] - 100.0, 2) if summary["equity_curve"] else 0.0,
        profit_factor=summary["profit_factor"],
        max_dd_pct=summary["mdd"],
    )
...
async def run_stock_walk_forward(request: Request, symbol: str = "NVDA") -> StockWalkForwardResponse:
    """
    RSI(2)均值回歸的穩健性驗證：整段~7年日線資料切成全期間/前50%/後50%/Q1-Q4，
    全部用同一組固定邏輯跑（沒有重新搜參數這回事），檢查勝率是否每個時間切片都
    撐得住50%，而不是只有全期間平均數字好看。公開端點，IP限流比照同一組規則。
    """
    client_ip = _backtest_client_ip(request)
    if _backtest_is_rate_limited(client_ip):
        raise HTTPException(status_code=429, detail=f"請求過於頻繁，每小時最多 {BACKTEST_RATE_LIMIT_MAX_REQUESTS} 次，請稍後再試。")
    ...
    if symbol_raw not in STOCK_MEANREV_SYMBOLS:
        raise HTTPException(
            status_code=400,
            detail=f"RSI(2)均值回歸目前只驗證過 {', '.join(STOCK_MEANREV_SYMBOLS)}，其他標的不提供。",
        )
